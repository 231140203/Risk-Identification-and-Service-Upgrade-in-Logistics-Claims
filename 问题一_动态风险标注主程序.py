...
def check_data_integrity(df, stage_name):
    print(f"\n=== {stage_name} 数据完整性检查 ===")
    print(f"总行数: {len(df)}")
...
def main():
    ...
    # 再次检查转换后的缺失值
    converted_missing = df_cleaned[['实际赔付金额', '索赔金额']].isna().sum()
    print(f"转换后仍有缺失值的行数: {converted_missing.sum()}")

    # 丢弃转换失败的记录
    df_cleaned = df_cleaned.dropna(subset=['实际赔付金额', '索赔金额'])
    ...
    for col in cap_:
        if col in df_cleaned.columns:
            ...
            print(f"{col} 盖帽边界: [{bound1}, {bound2}]")
            df_cleaned[col] = np.clip(df_cleaned[col], bound1, bound2)

    print(f"--- 预处理完成。原始数据: {initial_count} 行, 处理后: {len(df_cleaned)} 行。")
    ...
    print("最终分类结果 ---")
    print(percent_fin)
    print(f"分类后数据行数: {len(df_final)}")

    # 8. 修改输出部分 - 不要丢弃第一行
    outp_col = ['运单号', '实际赔付金额', '索赔金额', '索赔差额', 'P档', 'Y1', 'Y2', '风险类别']

    # 确保所有输出列都存在
    available_cols = [col for col in outp_col if col in df_final.columns]
    print(f"可用的输出列: {available_cols}")

    # 添加序号但不丢弃任何行
    df_final['序号'] = range(1, len(df_final) + 1)

    # 输出所有数据，不丢弃第一行
    output = df_final[available_cols].copy()

    print(f"准备输出的数据行数: {len(output)}")
    ...
    print("\n所有任务完成！")
    check_data_integrity(df, "读取原始数据后")
